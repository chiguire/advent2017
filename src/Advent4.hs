...
isAnagram x1 x2 = ((length x1) == (length x2)) && (all (\(c1, c2) -> c1 == c2) $ zip (sort x1) (sort x2))

getRepeatedAndAnagram :: [String] -> [String]
getRepeatedAndAnagram [] = []
getRepeatedAndAnagram (x:xs) 
    | x `elem` xs || (any (\s -> x `isAnagram` s) xs) = (x: getRepeatedAndAnagram xs)
    | otherwise   = getRepeatedAndAnagram(xs)
...
isValidPassword str = null $ getRepeated $ words str

isValidPassword2 str = null $ getRepeatedAndAnagram $ words str

-- Answers

advent4_1 = length $ filter (isValidPassword) $ lines input

advent4_2 = length $ filter (isValidPassword2) $ lines input

-- Input

input = [here|kvvfl kvvfl olud wjqsqa olud frc
